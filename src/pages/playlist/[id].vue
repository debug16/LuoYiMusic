...
onBeforeMount(() => {
  // 获取歌单详情
  playlistDetail(props.id)
    .then((res) => {
      if (res.code === 200)
        songsListDetails = res.playlist
    })
})
...
const playMusic = (songsList: any) => {
  ...
  if (song)
    playMusicStore.setPlayMusicList(songsList)
}
  ...
  <div v-show="songsListDetails" class="playlist" py-10>
    ...
    <div class="songsInfo" flex="~">
      <div class="songsImg" shrink-0 w="1/5" mr-15>
        <img
          w-full :src="`${songsListDetails?.coverImgUrl && songsListDetails?.coverImgUrl}?param=512y512`" alt=""
          rounded-xl
        >
      </div>
      <div class="songsContent" font-400 flex="~ col" justify-evenly items-start min-h-full>
        ...
        <div class="singer" text-lg>
          <p text-lg>
            Playlist by {{ songsListDetails?.creator?.nickname }}
          </p>
          <p class="time" text-sm color="#fff/68">
            最后更新于 {{ formatDate(songsListDetails?.updateTime) }}· {{ songsListDetails?.trackCount }} 首歌
          </p>
        </div>

        <div class="describe" text-sm color="#fff/68" :title="songsListDetails?.description">
          {{ songsListDetails?.description }}
        </div>
        <!-- 播放按钮 -->
        <div
          class="manipulate" flex="~" items-center space-x-1 bg="#bbcdff" py-2 px-4 rounded-lg color="#335eea"
          font-500 hover:scale-104 transition-transform @click="playMusic(songsList)"
        >
    ...
    <div class="songsList" mt-20 tracking-widest>
      <div
        v-for="songs in songsList" :key="songs.id" w-full flex="~" p-2 rounded-xl items-center
        :class="{ vip: songs.fee !== 0 && songs.fee !== 8, active: songs.id == playMusicStore.getPlayMusicId }"
        :title="songs.fee !== 0 && songs.fee !== 8 ? 'Only VIP Can Play' : ''" @dblclick="dblclickPlayMusic(songs)"
      >
        <div class="songs" flex="~ grow-0" items-center w="50%" pr-5>
          <div h-13 w-13 shrink-0 mr-5>
            <img h-full rounded-lg :src="`${songs?.al.picUrl}?param=224y224`" alt="">
          </div>
          <div overflow-hidden>
            <div truncate grow-0 w-full font-800 text-lg>
              <span>{{ songs.name }}</span>
              <span v-if="songs.alia[0]" text="#fff/60">
                ({{ songs?.alia && songs?.alia[0]?.toString() }})
              </span>
            </div>

            <div text-xs color="#fff/60" truncate>
        ...
        <div basis="10%" text-right>
          <span>{{ Math.floor(songs.dt / 1000 / 60) }}:{{ (Math.floor(songs.dt / 1000 % 60) + '').padStart(2, '0')
          }}</span>
        </div>
      </div>
    </div>
...
.songsList .active {
  background-color: rgba(140, 170, 255, 0.9) !important;
}
